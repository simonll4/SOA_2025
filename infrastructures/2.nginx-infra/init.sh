# ejecutar el script con sudo para poder acceder a fullchain.pem de certbot, ya que el archivo se crea con permisos de root.
# pendiente: hacer que el script se ejecute sin sudo.
...
CERT_FILE="/home/user/SOA2025/infra-nginx/certbot/conf/live/lpn3.crabdance.com/fullchain.pem"
BACKUP_FILE="/home/user/SOA2025/infra-nginx/certbot/conf/live/lpn3.crabdance.com/fullchain.pem.backup"
...
CERT_PATH="/home/user/SOA2025/infra-nginx/certbot/conf/live/lpn3.crabdance.com/fullchain.pem"
if [ -f "$CERT_PATH" ]; then
...
if ! crontab -l | grep -qE "0 4 \* \* 0 $RENEW_CERT_SCRIPT >> /var/log/check-renew-cert\.log 2>&1"; then
    ...
    (
        ...
        echo "$CRON_JOB"
    ) | crontab -# COLORES PARA LOS MENSAJES
GREEN='\033[38;5;46m'
NC='\033[0m' # Sin color
