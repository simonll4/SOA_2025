...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <nav class="mt-8">
        <div class="px-4 space-y-2">
          <RouterLink
            ...
            :class="{ 'bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-200': $route.path === '/dashboard' }"
          >
      ...
      <div class="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-200 dark:border-gray-700">
        <div class="flex items-center">
          <div class="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center">
            <span class="text-white text-sm font-medium">{{ userInfo?.username?.charAt(0).toUpperCase() }}</span>
          </div>
          <div class="ml-3">
            <p class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ userInfo?.username }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400">{{ isAdmin ? 'Administrador' : 'Operador' }}</p>
          </div>
          <button @click="logout" class="ml-auto p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
    ...
    <div class="lg:pl-64 flex flex-col min-h-screen">
      <!-- TOP BAR -->
      <div class="sticky top-0 z-40 flex h-16 items-center gap-x-4 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
        <button @click="toggleSidebar" class="lg:hidden -m-2.5 p-2.5 text-gray-700 dark:text-gray-200">
        ...
        <div class="flex flex-1 gap-x-4 self-stretch lg:gap-x-6">
          ...
          <div class="ml-auto flex items-center gap-x-4 lg:gap-x-6">
            <button @click="toggleTheme" class="p-2 text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
